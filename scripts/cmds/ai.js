...
module.exports = {
  ...
  try {
    ...
    const command = event.body.substring(prefix.length).trim();

    if (command === 'test') {
      await message.reply("A.I. is Alive");
      return;
    }
    ...
    api.sendMessage(
      `${error.message}.\n\nYou can try typing your question again or resending it, as there might be a bug from the server that's causing the problem. It might resolve the issue. If the problem hasn't resolved please contact the owner to get fixed immediately`,
      event.threadID
    );
  }
},
